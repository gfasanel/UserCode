    ...
    energyCorrectionMethod("DaunceyAndKenzie"), energyCorrected(0), energyCorrectedError(0)
{
    ...
    doMinvCut=false;
    //ptJets_ttH_thresh=20.;//
    ptJets_ttH_thresh=30.;//conta nel file .dat non qui!!
...
{
    cout<<"dentro analysis"<<endl;//MARKER
    if(PADEBUG)
...
bool PhotonAnalysis::VHhadronicBtag2012(LoopAll& l, int diphotonVHhadBtag_id, float* smeared_pho_energy, bool nm1, float eventweight, float myweight,bool *jetid_flags ){
    ...
    /*GIUSEPPE*/
bool PhotonAnalysis::LooseTag2012(LoopAll& l){
    bool tag= true;
    cout<<"Sono dentro LooseTag2012"<<endl;
    return tag;
}

bool PhotonAnalysis::TprimehadronicTag2012(LoopAll& l, int diphotonTprimehad_id, float* smeared_pho_energy, bool nm1, float eventweight, float myweight,bool *jetid_flags ){
    //giuseppe
    bool tag = false;
    
    l.isLep_ele=0;//GIUSEPPE=>cura i casi in cui il leptone viene trovato ma magari non passa i tagli successivi e gli resterebbe =1
    l.isLep_mu=0;//GIUSEPPE

    if(diphotonTprimehad_id==-1) return tag;//forse devo cambiare questo=> si, dentro analysis_settings.dat in common
  ...
  //////////////////Defining Tprime selection///////////////
    ...
    cout<<"prima delle variabili Tprime adronico"<<endl;
    ...
    cout<<"isLep_ele"<<l.isLep_ele<<endl;
    cout<<"isLep_mu"<<l.isLep_mu<<endl;
    //photon cuts
    //       ptLead_thresh=0.*diphoton.M()/120.;//selezione LOOSE
    //       ptSublead_thresh=0.*diphoton.M()/120.;//selezione LOOSE

    //    ptLead_thresh=77*diphoton.M()/120.;//REAL selection
    //    ptSublead_thresh=27.; // *diphoton.M()/120.;    //REAL selection

    ptLead_thresh=88*diphoton.M()/120.;//REAL selection
    ptSublead_thresh=35.; // *diphoton.M()/120.;    //REAL selection

    ptLeadTrig_thresh=33.;//nessun cambio
    ptSubleadTrig_thresh=25.;//nessun cambio

    //jet cuts
    //    nJets_thresh=njets_tthHad_thresh;//>=
    nJets_thresh=2;//>= GIUSEPPE :N.B. viene inserito perche' nell'ottimizzazione c'e' questa richiesta
    ptJets_thresh=ptJets_ttH_thresh;//25 GeV!! la stessa di RedN...
    //   Ht_thresh=0;//selezione LOOSE
    //        nbJets_thresh=0;//selezione LOOSE

    //    Ht_thresh=789;//REAL selection
    //    nbJets_thresh=1;//>= //REAL selection ||set d=> medium

    Ht_thresh=1000;//REAL selection
    nbJets_thresh=1;//>= //REAL selection ||set c=> loose
...
std::cout<<"pt: "<<p4_jet->Pt()<<" btag_loose "<<njets_btagloose<<" btag_medium "<<njets_btagmedium<<std::endl;
    ...
    Ht+=lead_p4.Pt() + sublead_p4.Pt();//GIUSEPPE CONTROLLARE
    //Ht+=l.met_pfmet;//GIUSEPPE CONTROLLARE
    //in hadronic no leptons
    ...
    //doing the selection  //togliere il controllo su njets!!!!//GIUSEPPE
       //bool hasPassedJetSelection= (njets_btagloose>=nbJets_thresh && Ht>=Ht_thresh && njets>=2);//selezione LOOSE
    ...
    //      if(tag==true)  cout<<"tagged Tprimehad , event"<<l.event<<"run "<<l.run<<" lumi "<<l.lumis<<endl;

    /*GIUSEPPE: END OF TPRIME HADRONIC*/
    cout<<"njets"<<njets<<endl;
    cout<<"isBtaggedMedium"<<isBtaggedMedium<<endl;
    cout<<"njets_loose"<<njets_btagloose<<endl;
    cout<<"Ht"<<Ht<<endl;
    cout<<"lead_p4.Pt()"<<lead_p4.Pt()<<endl;
    cout<<"lead_p4.Eta()"<<lead_p4.Eta()<<endl;
    cout<<"lead_p4.Phi()"<<lead_p4.Phi()<<endl;
    cout<<"lead_p4.M()"<<lead_p4.M()<<endl;
    cout<<"sublead_p4.Pt()"<<sublead_p4.Pt()<<endl;
    cout<<"sublead_p4.Eta()"<<sublead_p4.Eta()<<endl;
    cout<<"sublead_p4.Phi()"<<sublead_p4.Phi()<<endl;
    cout<<"sublead_p4.M()"<<sublead_p4.M()<<endl;

    if(tag){cout<<"e un Tprime adronico"<<endl;}
    return tag;
    
}

bool PhotonAnalysis::TprimeleptonicTag2012(LoopAll& l, int diphotonTprimelep_id, float* smeared_pho_energy, bool nm1, float eventweight, float myweight,bool *jetid_flags ){
    //giuseppe     
    bool tag = false;
    ...
    if(diphotonTprimelep_id==-1) return tag;//forse lo devo cambiare?????
    if(FMDEBUG)
	std::cout<<"----------------this is Tprime lep"<<std::endl;

    int diphoton_id=diphotonTprimelep_id;
	...
	if(l.ElectronMVACuts(elInd, elVtx)){
		...
		diphotonTprimelep_id = l.DiphotonCiCSelectionEleVeto( l.phoSUPERTIGHT, l.phoSUPERTIGHT, leadEtTprimelepCut,subleadEtTprimelepCut, 4,
								   applyPtoverM, &smeared_pho_energy[0], true, -1, veto_indices,true);
	    }
	    
	    
	    if(diphotonTprimelep_id!=-1)passElePhotonCuts=true;
	}
    }
    ...
    if(diphotonTprimelep_id !=-1 && elVtx != -1){

	lead_p4 = l.get_pho_p4( l.dipho_leadind[diphotonTprimelep_id], elVtx, &smeared_pho_energy[0]);
	sublead_p4 = l.get_pho_p4( l.dipho_subleadind[diphotonTprimelep_id], elVtx, &smeared_pho_energy[0]);
	diphoton = lead_p4+sublead_p4;

    }else if(diphotonTprimelep_id !=-1){

	lead_p4 = l.get_pho_p4( l.dipho_leadind[diphotonTprimelep_id], l.dipho_vtxind[diphotonTprimelep_id], &smeared_pho_energy[0]);
	sublead_p4 = l.get_pho_p4( l.dipho_subleadind[diphotonTprimelep_id], l.dipho_vtxind[diphotonTprimelep_id], &smeared_pho_energy[0]);
	diphoton = lead_p4+sublead_p4;
	...
	if(passMuPhotonCuts && passElePhotonCuts){
		...
		diphotonTprimelep_id=diphoton_id;
		l.isLep_mu=1;
		l.mu_ind=muonInd;
		l.el_ind=-1;
	    }else{
    ...
    l.diphoton_id_lep=diphotonTprimelep_id;
    ...
    /*    if(doDrGsfTrackCut){
	int leadind=l.dipho_leadind[diphotonTprimelep_id];
	int subleadind=l.dipho_subleadind[diphotonTprimelep_id];    
    ...
    */
    ...
    //    ptLead_thresh=49*diphoton.M()/120.;//REAL selection
    //    ptSublead_thresh=25.;//*diphoton.M()/120.;//REAL selection

    ptLead_thresh=59*diphoton.M()/120.;//REAL selection
    ptSublead_thresh=25.;//*diphoton.M()/120.;//REAL selection
    ...
    nJets_thresh=2;//>=
    ptJets_thresh=ptJets_ttH_thresh;//lascio quella di ttH 25 GeV

    //Ht_thresh=0;//selezione LOOSE
    //nbJets_thresh=0;//selezione LOOSE

    //   Ht_thresh=584;//REAL selection
    //nbJets_thresh=1;//>=//REAL selection

    Ht_thresh=770;//REAL selection
    nbJets_thresh=0;//>=//REAL selection
    ...
    cout<<"pt del leptone"<<lep->Pt()<<endl;//ANDRo fino in fondo                                                                                            
    cout<<"Eta del leptone"<<lep->Eta()<<endl;//ANDRo fino in fondo                                                                                          
    cout<<"phi del leptone"<<lep->Phi()<<endl;//ANDRo fino in fondo                                                                                          
    cout<<"m del leptone"<<lep->M()<<endl;//ANDRo fino in fondo 
    ...
    Ht+=lead_p4.Pt() + sublead_p4.Pt();//GIUSEPPE CONTROLLARE
    //    Ht+=l.met_pfmet;//GIUSEPPE controllare!!!!
    //double ele_pt,mu,pt=0;
    /*if(l.isLep_ele){
	ele_pt=el_tag->Pt();//CONTROLLA
    }
    if(l.isLep_mu){
	mu_pt=mu_tag->Pt();//CONTROLLA
    }
    if(ele_pt>mu_pt){
	Ht+=ele_pt;
    ...
    //doing the selection
    //bool hasPassedJetSelection= (njets_btagmedium>=nbJets_thresh && Ht>=Ht_thresh &&njets>=2);//selezione LOOSE
    bool hasPassedJetSelection= (njets>=nJets_thresh && njets_btagloose>=nbJets_thresh && Ht > Ht_thresh);//REAL selection
    ...
    if (FMDEBUG && tag==true) cout<<"tagged Tprimelep"<<endl;
    //    if(tag==true)  cout<<"tagged Tprimehad , event"<<l.event<<"run "<<l.run<<" lumi "<<l.lumis<<endl;

    /*if(tag==true){//GIUSEPPE
    cout<<njets<<"PHOT"<<endl;
    cout<<tag<<"TAG"<<endl;
    cout<<"ELE"<<l.isLep_ele<<endl;
    cout<<"MU"<<l.isLep_mu<<endl;
    }*/
    cout<<"njets"<<njets<<endl;
    cout<<"isBtaggedMedium"<<isBtaggedMedium<<endl;
    cout<<"isBtaggedLOOSE"<<isBtaggedLoose<<endl;
    cout<<"njets_loose"<<njets_btagloose<<endl;
    cout<<"Ht"<<Ht<<endl;
    cout<<"lead_p4.Pt()"<<lead_p4.Pt()<<endl;
    cout<<"lead_p4.Eta()"<<lead_p4.Eta()<<endl;
    ...
    cout<<"sublead_p4.Phi()"<<sublead_p4.Phi()<<endl;
    cout<<"sublead_p4.M()"<<sublead_p4.M()<<endl;
    if(tag){cout<<"e un Tprime leptoncio"<<endl;}
    return tag;
    /*END OF TPRIME LEPTONIC: GIUSEPPE*/

}
/*END of TPRIME: GIUSEPPE*/

/*TTH standard by francesco*/
...
bool PhotonAnalysis::TTHhadronicTag2012(LoopAll& l, int diphotonTTHhad_id, float* smeared_pho_energy, bool nm1, float eventweight, float myweight,bool *jetid_flags ){
	...
	cout<<"pt del jet"<<p4_jet->Pt()<<endl;
	njets++;
...
bool PhotonAnalysis::TTHleptonicTag2012(LoopAll& l, int diphotonTTHlep_id, float* smeared_pho_energy, bool nm1, float eventweight, float myweight,bool *jetid_flags ){
    ...
    //cout<<"VEDIAMO un po2: ptJet_thresh"<<ptJets_ttH_thresh<<endl;//20 GeV!! la stessa di RedN...
    l.isLep_mu=0;
    l.isLep_ele=0;
    l.el_ind=-1;
    l.mu_ind=-1;
    ...
    */
    ...
    cout<<"diphoton.M()"<<diphoton.M()<<endl;
    //jet cuts
    nJets_thresh=2;//era 2 prima, ma vediamo. Oramai posso credere a qualunque cosa!
    ptJets_thresh=ptJets_ttH_thresh;
    cout<<"ptjet_thresh"<<ptJets_thresh<<endl;
	...
	if(p4_jet->Pt()<ptJets_thresh) continue;
	cout<<"p4_jet->Pt()"<<p4_jet->Pt()<<endl;
	cout<<"p4_jet->eta()"<<p4_jet->Eta()<<endl;
	cout<<"p4_jet->phi()"<<p4_jet->Phi()<<endl;
	cout<<"p4_jet->m()"<<p4_jet->M()<<endl;
...
float PhotonAnalysis::BtagReweight(LoopAll &l, bool shiftBtagEffUp_bc, bool shiftBtagEffDown_bc, bool shiftBtagEffUp_l, bool shiftBtagEffDown_l){
	    int nBjets=0,nCjets=0,nLjets=0; 
	    /*	    float eff_b=0.80,eff_c=0.39,eff_l=0.13;
	      float SFb=1.008,SFc=1.008,SFl=1.09;*/
	    //medium WP
	    float eff_b=0.675,eff_c=0.08,eff_l=0.016;
	    float SFb=0.963,SFc=0.963,SFl=1.16;
...
void PhotonAnalysis::saveDatCardTree(LoopAll &l, int cur_type, int category, int inc_cat, float evweight, int ipho1, int ipho2, int ivtx, TLorentzVector lead_p4, TLorentzVector sublead_p4, bool isCutBased, double sigmaMrv, double sigmaMwv, double sigmaMeonly, float vtxP, string trainPhil, float lead_id_mva, float sublead_id_mva){
   ...
   if (l.signalNormalizer->GetProcess(cur_type)=="tth") proc_id=3;//che fare?
   if (l.signalNormalizer->GetProcess(cur_type)=="Tprime") proc_id=4;//proviamo cosi
