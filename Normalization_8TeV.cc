...
void Normalization_8TeV::Init7TeV(){
    ...
    for (std::map<double, double>::const_iterator iter = XSectionMap_ggh.begin(); iter != XSectionMap_ggh.end(); ++iter)
      XSectionMap_graviton[iter->first]=iter->second+XSectionMap_vbf[iter->first]+XSectionMap_wzh[iter->first]+XSectionMap_tth[iter->first]+XSectionMap_TprimeM400[iter->first];//GIUSEPPE//non serve?
...
void Normalization_8TeV::FillSignalTypes(){
  ...
  //Questo serve a dare i nomi all'istogramma di output
  //  SignalTypeMap[-100]=std::make_pair<TString,double>("Tprime",120);//GIUSEPPE
  SignalTypeMap[-100]=std::make_pair<TString,double>("TprimeM400",120);//GIUSEPPE
  SignalTypeMap[-15]=std::make_pair<TString,double>("TprimeM450",120);//GIUSEPPE
  SignalTypeMap[-16]=std::make_pair<TString,double>("TprimeM500",120);//GIUSEPPE
  SignalTypeMap[-17]=std::make_pair<TString,double>("TprimeM550",120);//GIUSEPPE
  //  SignalTypeMap[-]=std::make_pair<TString,double>("TprimeM600",120);//GIUSEPPE
  //SignalTypeMap[-]=std::make_pair<TString,double>("TprimeM650",120);//GIUSEPPE
  //non era -20; era -106
  SignalTypeMap[-18]=std::make_pair<TString,double>("TprimeM700",120);//GIUSEPPE
  SignalTypeMap[-19]=std::make_pair<TString,double>("TprimeM800",120);//GIUSEPPE
  ...
  //  SignalTypeMap[-101]=std::make_pair<TString,double>("ggh",125); Tanto non li uso => mi sono fregato il numeretto
...
{//Non lo modifico, ma tanto mi serve
...
double Normalization_8TeV::GetXsection(double mass, TString HistName) {
  ...
  if (HistName.Contains("ggh")) {
    ...
    std::cout << "Warning ggh, vbf, wh, zh, wzh, tth or Tprime  not found in histname!!!!" << std::endl;
    //exit(1);
  }
...
void Normalization_8TeV::PlotExpected(double Min, double Max){//Non penso che serva questo metodo
	...
	leg->AddEntry(Tprime,"T#bar{T} assoc","L");

	ggh->SetLineWidth(3);vbf->SetLineWidth(3);wzh->SetLineWidth(3);tth->SetLineWidth(3);Tprime->SetLineWidth(3);
	int i=0;
	for(double mH = Min;mH<=Max;mH+=0.5){
		...
		Tprime->SetPoint(i,mH,GetBR(mH)*GetXsection(mH,"Tprime"));//che non ho messo
		
		i++;
	}
	can->SetLogy();
	ggh->Draw("AL");vbf->Draw("L");wzh->Draw("L");tth->Draw("L");Tprime->Draw("L");
